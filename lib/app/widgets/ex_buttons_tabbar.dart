// ignore_for_file: omit_local_variable_types, curly_braces_in_flow_control_structures, use_named_constants, avoid_redundant_argument_values, always_declare_return_types, cast_nullable_to_non_nullable, prefer_final_fields, unnecessary_new, library_private_types_in_public_api, prefer_asserts_in_initializer_lists, require_trailing_commas, prefer_null_aware_method_calls, unnecessary_parenthesis, prefer_const_constructors
...
class ExButtonsTabBar extends StatefulWidget implements PreferredSizeWidget {
  ExButtonsTabBar({
    Key? key,
    required this.tabs,
    this.controller,
...
class _ButtonsTabBarState extends State<ExButtonsTabBar> with TickerProviderStateMixin {
  ...
  void initState() {
    super.initState();
    WidgetsBinding.instance.addPostFrameCallback((_) => _getCenterPadding(context));
  ...
  @override
  void didUpdateWidget(ExButtonsTabBar oldWidget) {
  ...
  _getCenterPadding(BuildContext context) {
    ...
    final RenderBox tabsParent = _tabsParentKey.currentContext!.findRenderObject() as RenderBox;
    final double screenWidth = tabsParent.size.width;

    RenderBox renderBox = _tabKeys.first.currentContext?.findRenderObject() as RenderBox;
  ...
  Widget _buildButton(
    ...
    final BoxDecoration? boxDecoration = BoxDecoration.lerp(
      BoxDecoration(
        color: widget.unselectedDecoration?.color ?? widget.unselectedBackgroundColor ?? Colors.grey[300],
        boxShadow: widget.unselectedDecoration?.boxShadow,
        gradient: widget.unselectedDecoration?.gradient,
        borderRadius: BorderRadius.circular(widget.radius),
      ),
      BoxDecoration(
        color: widget.decoration?.color ?? widget.backgroundColor ?? Theme.of(context).colorScheme.secondary,
            ...
            child: Row(
              children: <Widget>[
                ...
                tab.icon != null
                    ...
                    : Container(),
                SizedBox(
                  width: tab.icon == null || (tab.text == null && tab.child == null) ? 0 : widget.labelSpacing,
                ),
                // ignore: prefer_if_elements_to_conditional_expressions
                tab.text != null
                    ? Text(
  ...
  _handleTabAnimation() {
    _aniIndex = ((_controller!.animation!.value > _prevAniValue) ? _controller!.animation!.value : _prevAniValue).round();
  ...
  _scrollTo(int index) {
    ...
    if (offset < 0) {
      ...
      renderBox = (_textLTR ? _tabKeys.first : _tabKeys.last).currentContext?.findRenderObject() as RenderBox;
      ...
      // get the last button
      renderBox = (_textLTR ? _tabKeys.last : _tabKeys.first).currentContext?.findRenderObject() as RenderBox;
      // get its position
      position = renderBox.localToGlobal(tabsContainerOffset).dx;
      // and size
      size = renderBox.size.width;
      ...
      if (!widget.center && position + size - offset < screenWidth) offset = position + size - screenWidth;
    }
    offset *= (_textLTR ? 1 : -1);
